...
function ohSnap(text, options) {
  ...
  var $container = $('#'+options['container-id']),
    ...
    var bottom;
    var dur;
    var countL = $('.notifAlert').length;
    var href = options.href
    if(countL > 0){
        bottom = 30 * countL * 2 + 30;
        dur = parseInt(options.duration) + parseInt(countL) * 1000;
        html = $('<a href="'+BASE_URL+ href +'"><div style="display: block; bottom: '+bottom+'px !important" class="'+bottom+' notifAlert alert ' + color_markup + '">' + icon_markup + text + '</div></a>').fadeIn(options['fade-duration']);
    }else{
        bottom = 0;
        dur = options.duration;
        html = $('<a href="'+BASE_URL+ href +'"><div class="notifAlert alert '+ bottom +' '+ color_markup + '">' + icon_markup + text + '</div></a>').fadeIn(options['fade-duration']);
    }
